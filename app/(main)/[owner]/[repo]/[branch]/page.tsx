...
import { isConfigEnabled } from "../../../../../lib/config.ts";
import {
  Empty,
  EmptyContent,
  EmptyDescription,
...
export default function Page() {
  ...
  useEffect(() => {
    if (config?.object.content?.[0]) {
      router.replace(
        `/${config.owner}/${config.repo}/${encodeURIComponent(config.branch)}/${
    ...
    hasGithubIdentity(user) ? (
      <Empty className="absolute inset-0 border-0 rounded-none">
        <EmptyHeader>
          ...
          <EmptyDescription>
            This repository is not configured, and configuration access is disabled here. Edit
            &quot;.pages.yml&quot; on GitHub if you think this is a mistake.
          </EmptyDescription>
        </EmptyHeader>
        <EmptyContent>
          <Link
            className={buttonVariants({ variant: "default" })}
            href={`https://github.com/${config?.owner}/${config?.repo}/edit/${encodeURIComponent(
              config!.branch,
            )}/.pages.yml`}
          >
      ...
      <Empty className="absolute inset-0 border-0 rounded-none">
        <EmptyHeader>
          ...
          <EmptyDescription>
            This repository does not have a &quot;.pages.yml&quot; file yet. Ask a GitHub admin to
