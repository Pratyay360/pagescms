...
const schema = (field: Field) => {
  const normalizedValues = Array.isArray(field.options?.values)
    ? field.options.values.map((item) =>
        typeof item === "object" ? String(item.value ?? item.name ?? "") : String(item),
      )
    : [];
  ...
  if (field.options?.multiple) {
    ...
    if (min !== undefined) {
      zodSchema = zodSchema.min(min, `Select at least ${min} option${min === 1 ? "" : "s"}`);
    }
    if (max !== undefined) {
      zodSchema = zodSchema.max(max, `Select at most ${max} option${max === 1 ? "" : "s"}`);
    }
  ...
  return z.preprocess(
    ...
    field.required ? optionSchema : z.union([z.literal(""), optionSchema]).optional(),
  );
};
