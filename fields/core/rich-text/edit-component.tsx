...
import { Editor, type ImagePickerContext } from "../../../components/ui/editor/index.tsx";
import { Textarea } from "../../../components/ui/textarea.tsx";
import { Skeleton } from "../../../components/ui/skeleton.tsx";
import { cn } from "../../../lib/utils.ts";
import { MediaDialog, type MediaDialogHandle } from "../../../components/media/media-dialog.tsx";
...
import type { ApiResponse, FileSaveData } from "../../../types/api.ts";
...
const findMatchingMarkdownBracket = (input: string, start: number, open: string, close: string) => {
  let depth = 1;
...
const rewriteMarkdownImagesSync = (markdown: string, transformUrl: (url: string) => string) => {
...
const EditComponent = forwardRef((props: EditProps, ref: React.Ref<HTMLDivElement>) => {
  ...
  const { value, field, onChange, name, registerBeforeSubmitHook, onChangeRegistered } = props;
  void ref;
  ...
  const allowedExtensions = useMemo(() => {
    ...
    if (Array.isArray(mediaExtensions) && mediaExtensions.length > 0) {
      extensions = extensions.filter((extension) => mediaExtensions.includes(extension));
    }
  ...
  const toDisplayImageUrl = useCallback(
    async (url: string) => {
      ...
      const canonicalOutputPath = swapPrefix(decodedUrl, mediaConfig.input, mediaConfig.output);

      const inputPath = swapPrefix(decodedUrl, mediaConfig.output, mediaConfig.input, true);
      const normalizedInputPath = normalizePath(inputPath);
      const mediaInputRoot = normalizePath(mediaConfig.input);
      if (isAbsolutePath(decodedUrl) && !normalizedInputPath.startsWith(mediaInputRoot)) {
        return url;
      }
  ...
  const toCanonicalImageUrl = useCallback(
    (url: string) => {
      ...
      const relativePath = getRelativeUrl(config.owner, config.repo, config.branch, url, false);
      const normalizedRelativePath = normalizeMediaPath(decodePathSafely(relativePath));

      return swapPrefix(normalizedRelativePath, mediaConfig.input, mediaConfig.output);
    },
    [config, mediaConfig],
  );
  ...
  const editorToSource = useCallback(
    async (editorContent: string) => {
      if (!mediaConfig || !config || !editorContent) {
  ...
  const handleUploadImage = useCallback(
    async (file: File) => {
      ...
      const uploadFilename = getUploadFileName(file.name, options.rename ?? mediaConfig.rename);
      const targetPath = joinPathSegments([rootPath ?? mediaConfig.input, uploadFilename]);
    ...
    [allowedExtensions, config, mediaConfig, options.rename, rootPath, toDisplayImageUrl],
  );
  ...
  return (
    <div className="space-y-2">
      ...
      {showSwitcher &&
        labelSlotEl &&
        createPortal(<div className="flex items-center">{switcherNode}</div>, labelSlotEl)}
