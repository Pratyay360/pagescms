...
} from "../../../components/ui/editor/index.tsx";
...
} from "../../../components/media/media-dialog.tsx";
  ...
  (props: EditProps, ref: React.Ref<HTMLDivElement>) => {
    ...
    const allowedExtensions = useMemo(() => {
      ...
      if (Array.isArray(options.extensions) && options.extensions.length > 0) {
        ...
        extensions = options.categories.flatMap((category) =>
    ...
    const sourceToEditor = useCallback(
      async (source: string) => {
        ...
        const withInputPrefixInMd = rewriteMarkdownImagesSync(
          source,
          (url) => swapPrefix(url, mediaConfig.output, mediaConfig.input, true),
        );
