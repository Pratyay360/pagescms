...
import { isCacheEnabled, isConfigEnabled } from "../../lib/config.ts";
import { getRootActions } from "../../lib/actions.ts";
import { getVisits } from "../../lib/tracker.ts";
import { RepoActionButtons } from "./repo-action-buttons.tsx";
...
export function RepoSidebar() {
  ...
  const rootActions = useMemo(() => getRootActions(config?.object), [
    config?.object,
  ]);
  ...
  return (
    <Sidebar collapsible="offcanvas">
      ...
      <SidebarContent>
        {groups.map((group, index) => <Fragment key={index}>{group}</Fragment>)}
      </SidebarContent>
