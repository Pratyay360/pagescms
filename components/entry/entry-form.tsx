...
import { editComponents } from "../../fields/registry.ts";
import {
  generateZodSchema,
  getDefaultValue,
  initializeState,
...
import { interpolate } from "../../lib/schema.ts";
...
const ListField = ({
  ...
  const supportsItemCollapse = field.type === "object" ||
    field.type === "block";
  const isCollapsible = !!(
  ...
  const shouldShowListHeader = field.label !== false || field.required ||
  ...
  const sortableItems = useMemo(() => arrayFields.map((item) => item.id), [
    arrayFields,
  ]);
          ...
          {shouldShowListHeader && (
            <div className="flex items-center h-5 gap-x-2">
              ...
              {isCollapsible &&
                ...
                (() => {
                  const isAllExpanded = openStates.length > 0 &&
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {isReadonly ? null : typeof field.list === "object" &&
                  field.list?.max &&
                  arrayFields.length >= field.list.max
                ? null
                : (
                  <Button
                    type="button"
...
const BlocksField = forwardRef<HTMLDivElement, NestedFieldProps>(
  (props, ref) => {
    ...
    const [isRemoveBlockDialogOpen, setIsRemoveBlockDialogOpen] = useState(
      false,
    );
    const isReadonly = Boolean(field.readonly);
    ...
    const selectedBlockDefinition = useMemo(() => {
      const definition = blocks.find((b: Field) =>
        b.name === selectedBlockName
      );
      return definition;
...
const SingleField = ({
  ...
  const shouldShowFieldMeta = showLabel &&
...
const EntryForm = ({
  ...
  const form = useForm({
    resolver: zodSchema ? zodResolver(zodSchema as never) : undefined,
    defaultValues,
  ...
  const handleSubmit = useCallback<SubmitHandler<Record<string, unknown>>>(
    async (values) => {
  ...
  const handleFormSubmit = useCallback(
    async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      await form.handleSubmit(
        handleSubmit as SubmitHandler<unknown>,
        handleError,
      )(event);
    },
